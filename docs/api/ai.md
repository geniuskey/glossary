...
`POST /chat`은 로그인 세션 또는 `read` scope API Key로 호출할 수 있다. API Key 호출은 대화 세션을
저장하지 않으며, 서버가 현재 용어집과 활성 검색 인덱스에서 근거를 찾아 관리자 화면에서 설정한
AI 모델로 답한다. 호출자는 용어집 본문을 프롬프트에 직접 붙여 넣지 않고 `question`만 보내면 된다.
