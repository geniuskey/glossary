...
> 현재 배포판은 **`0.2.0` 개발 미리보기**다. 기능 검토와 사내 파일럿에 사용하고,
> 업그레이드 전에는 반드시 DB 백업과 복구를 검증한다. 앱과 마이그레이터는 항상 같은
> 버전 조합으로 고정한다.

| 이미지 | 고정 태그 | 용도 |
|---|---|---|
| `euiyun/glossary` | `0.2.0` | Glossary 웹 애플리케이션 |
| `euiyun/glossary` | `0.2.0-migrator` | 앱 기동 전에 실행하는 DB 마이그레이션 |
...
```dotenv
GLOSSARY_IMAGE=euiyun/glossary:0.2.0
