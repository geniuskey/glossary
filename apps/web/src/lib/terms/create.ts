...
//
// R135: 화면 주소가 `/g/<slug>`로 옮겨가면서 용어와 위키의 네임스페이스를
// 분리했다. 그래도 "new"는 예약어로 남긴다 —
      ...
      const { term, savedSurfaces } = await db.transaction(async (tx) => {
        const [insertedTerm] = await tx
          ...
          .values({
            ...
            topic: input.tags !== undefined ? input.tags[0] ?? null : input.topic ?? null,
            tags: input.tags ?? (input.topic ? [input.topic] : []),
