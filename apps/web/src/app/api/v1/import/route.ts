...
import { applyImport, dryRunImport, MAX_IMPORT_ROWS } from "@/lib/import/apply";
...
// R119: Content-Length가 이 값을 넘으면 곧바로 거부한다. 실제 파일 크기와
// 달리 멀티파트 경계·필드 헤더만큼 살짝 부풀 수 있어 여유를 둔다(바이너리
// 파일 파트 자체는 base64로 부풀지 않는다).
...
export const POST = withApiErrors(async (request: Request) => {
  ...
  // 그 뒤에 `file.size > MAX_BYTES`를 검사하면(계획서 스케치) 이미 늦다. xlsx는
  // 압축률이 높아 20만 행이 10MB 안에 들어가므로, 상한이 장식이 되지 않으려면
  ...
  if (Number.isFinite(contentLength) && contentLength > MAX_BYTES + CONTENT_LENGTH_SLOP) {
