...
import { deleteDomain, renameDomain } from "@/lib/terms/domains";
...
const patchSchema = z.object({ label: z.string().trim().min(1).max(DOMAIN_VALUE_MAX) }).strict();
...
export const PATCH = withApiErrors(async (request: Request, context: { params: Promise<{ key: string }> }) => {
  ...
  const result = await renameDomain(key, parsed.data.label);
  if (result === "not_found") return apiError("not_found", "도메인을 찾을 수 없습니다.", 404);
  if (result === "duplicate") return apiError("operation_conflict", "같은 이름의 도메인이 이미 있습니다.", 409);
