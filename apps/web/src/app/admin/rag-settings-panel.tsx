        ...
        <div className="card overflow-hidden">
          ...
          <div className="grid gap-5 p-4 lg:grid-cols-2">
            <EndpointFields
              ...
              onHeadersChange={setEmbeddingHeaders}
            />
            <EndpointFields
              title="Reranker API"
              ...
              onHeadersChange={setRerankerHeaders}
            />
          </div>
        </div>
...
function EndpointFields({
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2"><h3 className="text-sm font-semibold text-ink">{title}</h3>{title === "Embedding API" && <HelpTip text="OpenAI-compatible /embeddings 또는 Gemini batchEmbedContents 응답을 지원합니다." />}</div>
      ...
      <label className="block"><span className="label">모델</span><input value={model} onChange={(event) => onModelChange(event.target.value)} disabled={disabled} className="field" autoComplete="off" />{dimensions && <span className="mt-1 block text-[11px] text-ink-3">출력 차원: {dimensions}</span>}</label>
      <label className="block"><span className="label">API Base URL</span><input type="url" value={baseUrl} onChange={(event) => onBaseUrlChange(event.target.value)} disabled={disabled} className="field font-mono text-xs" autoComplete="off" /></label>
