...
export default async function NewWikiPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <AppShell user={user} title="새 위키 문서" current="wiki" roomy>
    <div className="space-y-5">
      ...
      <WikiEditor initialPage={initialSourceUrl ? { title: "", summary: null, sourceUrl: initialSourceUrl, domain: [], status: "draft", terms: [] } : null} domains={domains.map((domain) => ({ key: domain.key, label: domain.label }))} />
    </div>
  </AppShell>;
}
