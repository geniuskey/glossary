...
test("사이드바 탐색과 상단 검색·생성·계정 영역을 분리한다", async () => {
  ...
  expect(html).toContain('aria-label="Glossary 앱 버전 v0.3.1"');
  ...
  expect(html).not.toContain('sidebar-expanded-only ml-auto hidden text-[10px]');
...
test("설정과 관리자, 도움말 링크는 개인 계정 하위 메뉴에 있다", () => {
  ...
  expect(html).not.toContain('앱 버전 v0.3.1');
});
