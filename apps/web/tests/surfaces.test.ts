...
test("nameEn은 Type과 무관하게 canonical 표기로 파생된다", () => {
  const canonical = deriveSurfaces({ nameEn: "Auto Exposure" }, []);
  expect(canonical.find((s) => s.text === "Auto Exposure")?.kind).toBe("canonical");

  const identifier = deriveSurfaces({ nameEn: "AE" }, []);
...
test("표준 이름에서 파생된 표기는 명시 표기 목록에서 빠진다 (R110)", () => {
  const names = { nameEn: "Auto Exposure", nameKo: "자동노출" };
  const stored = [
...
test("사용자가 직접 추가한 표기는 명시 표기 목록에 남는다 (R110)", () => {
  const names = { nameEn: "Auto Exposure" };
  const stored = [
