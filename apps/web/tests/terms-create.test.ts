...
test("같은 정규화 키를 가진 기존 용어가 있으면 경고를 반환하되 저장은 한다", async () => {
  const first = await createTerm(
    { nameEn: "Auto Exposure", domain: ["ISP"], status: "active", surfaces: [] },
    null,
  );
  created.push(first.term.id);

  const second = await createTerm(
    { nameEn: "auto-exposure", domain: ["PM"], status: "active", surfaces: [] },
    null,
  );
...
test("슬러그가 겹치면 접미사를 붙여 고유하게 만든다", async () => {
  const a = await createTerm({ nameEn: "Gain", domain: [], status: "active", surfaces: [] }, null);
  const b = await createTerm({ nameEn: "Gain", domain: [], status: "active", surfaces: [] }, null);
...
test("트랜잭션 중간(리비전 insert)이 실패하면 terms/term_surfaces에도 아무 것도 남지 않는다 (R32)", async () => {
  ...
  try {
    await expect(
      createTerm(
        { nameEn: "Rollback Probe", domain: [], status: "active", surfaces: [] },
        null,
      ),
    ).rejects.toThrow();
...
test("bodyMd가 저장되고 그대로 반환된다 (R33)", async () => {
  const bodyMd = "# 제목\n\n본문 내용입니다.";
  const { term } = await createTerm(
    { nameEn: "Body Md Probe", domain: [], status: "active", surfaces: [], bodyMd },
    null,
  );
  created.push(term.id);
...
test("슬러그 경합이 나면 재시도해서 -2로 저장한다 (R48)", async () => {
  ...
  const blockerTx = db.transaction(async (tx) => {
    const [row] = await tx
      .insert(terms)
      .values({ slug: "retry-probe", nameEn: "Retry Probe Blocker", domain: [], status: "active" })
      .returning();
    created.push(row!.id);
    signalInserted();
  ...
  const createPromise = createTerm(
    { nameEn: "Retry Probe", domain: [], status: "active", surfaces: [] },
    null,
  );
...
// `/g/<slug>`로 옮겨가면서 그 충돌은 사라졌지만, next.config.ts의
...
test("R92: 이름이 New인 용어는 슬러그가 new가 되지 않는다", async () => {
  ...
  const { term } = await createTerm(
    { nameEn: "New", domain: [], status: "active", surfaces: [] },
    null,
  );
  created.push(term.id);
