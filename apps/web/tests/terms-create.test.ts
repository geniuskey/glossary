...
test("트랜잭션 중간(리비전 insert)이 실패하면 terms/term_surfaces에도 아무 것도 남지 않는다 (R32)", async () => {
  ...
  try {
    await expect(
      createTerm(
        { termType: "term", nameEn: "Rollback Probe", domain: [], status: "draft", surfaces: [] },
        null,
      ),
    ).rejects.toThrow();
...
test("bodyMd가 저장되고 그대로 반환된다 (R33)", async () => {
  const bodyMd = "# 제목\n\n본문 내용입니다.";
  const { term } = await createTerm(
    { termType: "term", nameEn: "Body Md Probe", domain: [], status: "draft", surfaces: [], bodyMd },
    null,
  );
  created.push(term.id);
...
test("슬러그 경합이 나면 재시도해서 -2로 저장한다 (R48)", async () => {
  ...
  const blockerTx = db.transaction(async (tx) => {
    const [row] = await tx
      .insert(terms)
      .values({ slug: "retry-probe", termType: "term", nameEn: "Retry Probe Blocker", domain: [], status: "draft" })
      .returning();
    created.push(row!.id);
    signalInserted();
  ...
  const createPromise = createTerm(
    { termType: "term", nameEn: "Retry Probe", domain: [], status: "draft", surfaces: [] },
    null,
  );
