...
> **Development preview — `0.1.1`**
>
> Grossary is under active development. Use this release for evaluation and internal pilots,
> pin both container tags to `0.1.1`, and keep tested database backups before upgrading.
>
> **개발 미리보기 — `0.1.1`**
>
> 현재 활발히 개발 중인 초기 버전입니다. 기능 검토와 사내 파일럿 용도로 사용하고,
> 앱과 마이그레이터 이미지를 모두 `0.1.1`로 고정한 뒤 업그레이드 전 백업을 보관하세요.
...
| Tag | Purpose |
|---|---|
| `0.1.1` | Version-pinned web application (recommended) |
| `0.1.1-migrator` | Matching database migrations (recommended) |
...
The environment template pins both images to the `0.1.1` development release. Set a long, URL-safe database password, then pull and start the stack:
...
|---|---|
| `GROSSARY_IMAGE` | Web image, for example `euiyun/grossary:0.1.1` |
| `GROSSARY_MIGRATOR_IMAGE` | Matching migration image, for example `euiyun/grossary:0.1.1-migrator` |
